...
const Header = () => {
  ...
  return (
    <header className='bg-white flex justify-center relative top-10  py-8 '>
      {/* Left side  */}
      <Link
        to='/'
      ...
      {/* Middle */}
      <h1 className='font-bold xxs:relative xxs:right-5 xxs:top-4 xs:text-4xl  lg:text-6xl  '>
        {' '}
        Pupper Newsfeed
      </h1>
      <Link to='/' className='xxs:relative xxs:right-0 sm:right-0'>
        <img src={dog} alt='dog' className='mx-5' />
      </Link>

      {/* Right Side */}
      <div className=' relative flex flex-col xs:left-300 lg:left-20 xl:left-60   2xl:left-80   '>
        ...
        <>
          {user?.state?.isSignedIn ? (
            ...
            <Link to='/authentication'>
              <button className='relative top-3 shadow bg-slate-700 hover:bg-slate-800 focus:shadow-outline focus:outline-none text-white font-bold  xs:py-2 xs:px-2  xxs:py-2 xxs:px-4 rounded '>
                Sign In
              </button>
            </Link>
          )}
